...
class Linear(DQN):
    ...
    def initialize_models(self):
        ...
        self.target_network.load_state_dict(self.q_network.state_dict()) # this is not neccessary
    ...
    def get_q_values(self, state: torch.Tensor, network: str = "q_network"):
        ...
        state_flatten = torch.flatten(state, start_dim=1) # keep batch_size
        network_to_use = self.target_network if network == "target_network" else self.q_network
        out = network_to_use(state_flatten)
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def calc_loss(
        ...
        ##############################################################
        ##################### YOUR CODE HERE - 3-5 lines #############
        # Q_samp(s) : current reward + potential maximum future reward
        target_next_q_values = target_q_values.max(dim=1).values
        # result of max function:
        # torch.return_types.max(
        #     values=tensor([0.2548, 0.9893, 1.5086, 0.6778, 1.4086]),
        #     indices=tensor([0, 3, 4, 4, 3]))
        # we only take values
        
        # we only add gamma if done_mask is False
        target_q_samp = rewards + gamma * target_next_q_values * torch.logical_not(done_mask)

        # note) 'a' in Q(s,a) is the action actually taken
        q_sa = q_values[torch.arange(q_values.size(dim=0)), actions.long()]
        # actions' type is float32, need to change the type

        loss = torch.nn.functional.mse_loss(target_q_samp, q_sa)
        return loss
