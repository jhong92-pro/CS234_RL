...
class BasePolicy:
    ...
    def act(self, observations, return_log_prob = False):
        ...
        if return_log_prob:
            log_probs = action_distribution.log_prob(sampled_actions).cpu().detach().numpy()
...
class GaussianPolicy(BasePolicy, nn.Module):
    def __init__(self, network, action_dim):
        ...
        self.log_std = nn.Parameter(torch.zeros(action_dim.shape[0]))
        #######################################################
        #########          END YOUR CODE.          ############
    ...
    def action_distribution(self, observations):
        ...
        # note that) diagonal: Cov[Yi,Yj]=0 if i≠j
        loc = self.network(observations)
        scale = self.std()
        cov = torch.diag(scale**2)
        distribution = torch.distributions.MultivariateNormal(loc,cov)
        #######################################################
        #########          END YOUR CODE.          ############
        return distribution
